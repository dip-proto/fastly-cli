...
// AuthInitialized returns true if the [auth] section has been set up.
func (f *File) AuthInitialized() bool {
	return len(f.Auth.Tokens) > 0
}

// GetAuthToken returns the named auth token, or nil if not found.
...
// GetDefaultAuthToken returns the default auth token and its name.
func (f *File) GetDefaultAuthToken() (string, *AuthToken) {
	if f.Auth.Default == "" || f.Auth.Tokens == nil {
		return "", nil
	}
	if t, ok := f.Auth.Tokens[f.Auth.Default]; ok {
		return f.Auth.Default, t
	}
	return "", nil
}

// SetAuthToken stores or replaces a named token entry.
...
// DeleteAuthToken removes a named token entry. Returns false if not found.
...
// SetDefaultAuthToken sets the default token name.
