...
func ForbiddenRemediation() string {
	...
	if env.AuthCommandDisabled() {
		parts = append(parts, fmt.Sprintf("If your token is policy-restricted, use an unrestricted token via %s.", env.APIToken))
		parts = append(parts, "Check your current identity with `fastly whoami`.")
	} else {
		parts = append(parts, "If you use a stored token with a policy, check `fastly auth policy list` and `fastly auth show`.")
