...
func TestAuthAdd(t *testing.T) {
	scenarios := []testutil.CLIScenario{
		...
		{
			Name: "add infers readonly policy for global:read scope",
			Args: "add readonly-entry --api-token test-token-value",
			API: mock.API{
				GetCurrentUserFn: testGetCurrentUser,
				GetTokenSelfFn:   testTokenSelfReadOnly,
			},
			WantOutputs: []string{`Token "readonly-entry" added`, "Token saved to"},
			Validator: func(t *testing.T, _ *testutil.CLIScenario, opts *global.Data, _ *threadsafe.Buffer) {
				t.Helper()
				at := opts.Config.GetAuthToken("readonly-entry")
				if at == nil {
					t.Fatal("expected auth token to exist")
				}
				if len(at.Allow) != 1 || at.Allow[0] != "readonly" {
					t.Errorf("want Allow [readonly], got %v", at.Allow)
				}
			},
		},
		{
			Name: "add does not override explicit policy with readonly",
			Args: "add custom-entry --api-token test-token-value --policy service:write",
			API: mock.API{
				GetCurrentUserFn: testGetCurrentUser,
				GetTokenSelfFn:   testTokenSelfReadOnly,
			},
			WantOutputs: []string{`Token "custom-entry" added`, "Token saved to"},
			Validator: func(t *testing.T, _ *testutil.CLIScenario, opts *global.Data, _ *threadsafe.Buffer) {
				t.Helper()
				at := opts.Config.GetAuthToken("custom-entry")
				if at == nil {
					t.Fatal("expected auth token to exist")
				}
				if len(at.Allow) != 1 || at.Allow[0] != "service:write" {
					t.Errorf("want Allow [service:write], got %v", at.Allow)
				}
			},
		},
		{
...
func TestAuthLogin(t *testing.T) {
	scenarios := []testutil.CLIScenario{
		...
		{
			Name: "login infers readonly for global:read scope",
			Args: "login",
			API: mock.API{
				GetCurrentUserFn: testGetCurrentUser,
				GetTokenSelfFn:   testTokenSelfReadOnly,
			...
			WantOutputs: []string{"Authenticated as alice@example.com", "Token saved to"},
			Validator: func(t *testing.T, _ *testutil.CLIScenario, opts *global.Data, _ *threadsafe.Buffer) {
				t.Helper()
				at := opts.Config.GetAuthToken("readonly-token")
				if at == nil {
					t.Fatal("expected auth token 'readonly-token' to exist")
				}
				if len(at.Allow) != 1 || at.Allow[0] != "readonly" {
					t.Errorf("want Allow [readonly], got %v", at.Allow)
				}
		...
		{
			Name: "login overwrites existing token and preserves Allow policy",
			Args: "login",
			API: mock.API{
				GetCurrentUserFn: testGetCurrentUser,
				GetTokenSelfFn:   testTokenSelfReadOnly,
			...
			ConfigFile: &config.File{
				Auth: config.Auth{
					Default: "readonly-token",
					Tokens: config.AuthTokens{
						"readonly-token": &config.AuthToken{
							Type:  config.AuthTokenTypeStatic,
							Token: "old-token",
							Allow: []string{"service:write"},
						},
					},
				},
			},
			...
			Validator: func(t *testing.T, _ *testutil.CLIScenario, opts *global.Data, _ *threadsafe.Buffer) {
				...
				if len(at.Allow) != 1 || at.Allow[0] != "service:write" {
					t.Errorf("want Allow [service:write] (preserved), got %v", at.Allow)
				}
			},
		},
	}
