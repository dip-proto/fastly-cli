...
func TestSSO(t *testing.T) {
	scenarios := []testutil.CLIScenario{
		...
		// 9. SSO login with --token preserves existing Allow policy.
		{
			Args: "auth login --sso --token restricted",
			ConfigFile: &config.File{
				Auth: config.Auth{
					Default: "restricted",
					Tokens: config.AuthTokens{
						"restricted": &config.AuthToken{
							Type:             config.AuthTokenTypeSSO,
							Token:            "old-token",
							Email:            "test@example.com",
							...
							RefreshExpiresAt: "2020-01-01T00:00:00Z",
						},
					},
				},
			},
			...
			WantOutputs: []string{
				"We're going to authenticate the 'restricted' token",
				"has been stored. Use 'fastly auth list' to view tokens.",
				"Token saved to",
			},
			Validator: func(t *testing.T, _ *testutil.CLIScenario, opts *global.Data, _ *threadsafe.Buffer) {
				at := opts.Config.GetAuthToken("restricted")
				if at == nil {
					t.Fatal("expected auth token 'restricted' to exist")
				}
				if at.Token != "new-token-123" {
					t.Errorf("want token: new-token-123, got token: %s", at.Token)
				}
				if len(at.Allow) != 1 || at.Allow[0] != "readonly" {
					t.Errorf("want Allow: [readonly], got Allow: %v", at.Allow)
				}
			},
		},
		// 10. Migration before token resolution: legacy profiles are migrated to
		...
		// 19. Static token auto-prompt preserves existing Allow policy.
		{
			Name: "auto-prompt static preserves allow",
			Args: "whoami",
			API: mock.API{
				GetCurrentUserFn: func(_ context.Context) (*fastly.User, error) {
					return &fastly.User{
			...
			Validator: func(t *testing.T, _ *testutil.CLIScenario, opts *global.Data, _ *threadsafe.Buffer) {
				...
				if at.Token != "new-static-token" {
					t.Errorf("want token: new-static-token, got token: %s", at.Token)
				}
				if len(at.Allow) != 1 || at.Allow[0] != "readonly" {
					t.Errorf("want Allow: [readonly], got Allow: %v", at.Allow)
				}
		...
		// 20. FASTLY_USE_SSO=1 triggers SSO flow instead of token prompt.
		{
