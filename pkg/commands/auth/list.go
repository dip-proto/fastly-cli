...
// NewListCommand returns a new command registered in the parent.
...
func (c *ListCommand) Exec(_ io.Reader, out io.Writer) error {
	...
	for name, entry := range tokens {
		...
		if len(entry.Allow) > 0 {
			policyStr = fmt.Sprintf(" [policy: %s]", strings.Join(entry.Allow, ", "))
		}
		...
		text.Output(out, "%s%s (%s)%s%s\n", marker, name, info, policyStr, reauthStr)
	}
