...
// in the auth config as the default token. If a token with the derived name
// already exists, its Allow policy is preserved. Returns the stored name and
// metadata.
func StoreStaticToken(g *global.Data, token string) (name string, md *TokenMetadata, err error) {
	...
	if existing := g.Config.GetAuthToken(name); existing != nil {
		entry.Allow = existing.Allow
	}
