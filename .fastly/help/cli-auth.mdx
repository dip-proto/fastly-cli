...
A policy limits which commands a stored token can run (for example, read-only access). Policies only apply to stored auth tokens; `--token` (when available) and `FASTLY_API_TOKEN` bypass policy checks.

```
fastly auth policy list                              # see available permissions
fastly auth policy set --name staging --policy readonly     # restrict a token
fastly auth policy set --name staging --clear               # remove restrictions
```
