...
| `FASTLY_DISABLE_AUTH_COMMAND` | When set (any non-empty value), the `fastly auth` command tree and the `--token`/`-t` global flag are disabled. Authentication is handled via `FASTLY_API_TOKEN` or pre-configured stored tokens. Background SSO flows are unaffected. |
